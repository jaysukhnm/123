...
function createHomepageGoogleMap(_latitude,_longitude,json){
    $.get("js/external/_infobox.js", function() {
        gMap();
    });
    function gMap(){
    ...
    visibleItemsArray.push(
        '<li>' +
            '<div class="item" id="' + json.data[a].id + '">' +
                ...
                '</a>' +
                '<div class="wrapper">' +
                    '<div class="store_first_line col-md-12">'+
                        '<div class="store_name col-md-9"><a href="#" id="' + json.data[a].id + '"><h3>' + json.data[a].name + '</h3></a><span>(category)</span></div>' +
                        '<div class="store_distance col-md-3">25km</div>' +
                    '</div>'+
                    '<div class="store_address col-md-12">'+
                        '<div class="col-md-8">' + json.data[a].address + '</div>' +                        
                        '<div class="rating col-md-4" data-rating="' + json.data[a].rating + '"></div>' +
                    '</div>' +
                '</div>' +
            '</div>' +
        '</li>'
...
function multiChoice(sameLatitude, sameLongitude, json) {
    //if (clickedCluster.getMarkers().length > 1){
        ...
        $('.modal-window').load( 'js/external/_modal-multichoice.html', function() {
